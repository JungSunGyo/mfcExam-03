...
BEGIN_MESSAGE_MAP(CgPrjDlg, CDialogEx)
	...
	ON_BN_CLICKED(IDC_BTN_PAINT, &CgPrjDlg::OnBnClickedBtnPaint)
END_MESSAGE_MAP()
...
void CgPrjDlg::OnBnClickedBtnTest()
{
	...
	int nWidth = m_pDlgImage->m_image.GetWidth();  //640
	int nHeight = m_pDlgImage->m_image.GetHeight();  //480
	...
	for (int k = 0; k < 100; k++) {
		int x = rand() % nWidth;
		int y = rand() % nHeight;
		fm[y * nPitch + x] = rand()%0xff;  //rand()%0xff;
		//fm[y * nPitch + x+1] = 0;
		//fm[y+1 * nPitch + x] = 0;
		//fm[y+1 * nPitch + x+1] = 0;
	}
...
void CgPrjDlg::OnBnClickedBtnGetData()
{
	...
	m_pDlgImage->m_nCenterX = (int)dCenterX;
	m_pDlgImage->m_nCenterY = (int)dCenterX;
}
...
void CgPrjDlg::OnBnClickedBtnCircle()
{
	...
	memset(fm, 0x00, nWidth * nHeight);
	CString str;
	GetDlgItemTextW(IDC_EDIT_SIZE, str);
	int nData = _ttoi(str);
	m_pDlgImage->m_nRadius = nData;

	CRect rect(0, 0, nData * 2, nData * 2);
	for (int j = rect.top; j < rect.bottom; j++) {
		for (int i = rect.left; i < rect.right; i++) {
			if (IsInCircle(i, j, nData, nData, nData)) {
				fm[j * nPitch + i] = 0x80;
			}
		}
	}
...
void CgPrjDlg::OnBnClickedBtnPaint()
{
	// TODO: 여기에 컨트롤 알림 처리기 코드를 추가합니다.
	unsigned char* fm = (unsigned char*)m_pDlgImage->m_image.GetBits();
	int nWidth = m_pDlgImage->m_image.GetWidth();
	...
	int nPitch = m_pDlgImage->m_image.GetPitch();
	memset(fm, 0x00, nWidth * nHeight);
	m_pDlgImage->m_bDraw = true;
	m_pDlgImage->Invalidate();
}

bool CgPrjDlg::IsInCircle(int x, int y, int nCenterX, int nCenterY, int nRadius)
{
	bool bRet = false;
	double dX = (double)nCenterX - x;
	double dY = (double)nCenterY - y;
	double dDist = dX * dX + dY * dY;

	if (dDist < (double)nRadius * nRadius)
		bRet = true;
